...
USING (
    ...
    OR EXISTS (
        ...
        WHERE m.id = documents.matter_id
          AND (m.individual_user_id = auth.uid() OR m.business_id IN (SELECT b.id FROM public.businesses b WHERE b.owner_user_id = auth.uid()))
    )
    OR (
...
USING (
    ...
    OR (
        ...
        AND (
            (matter_id IS NOT NULL AND public.staff_can_access_matter(matter_id, 'view_matters'))
...
WITH CHECK (
    public.is_super_admin()
    OR (public.current_user_role() <> 'STAFF'::app_role AND created_by = auth.uid())
    OR (public.current_user_role() = 'STAFF'::app_role AND public.has_staff_permission('manage_assignments') AND created_by = auth.uid())
);
...
USING (
    ...
    OR (
        ...
        AND (created_by = auth.uid() OR assigned_staff_id IN (SELECT s.id FROM public.staff s WHERE s.user_id = auth.uid() AND s.is_active = true))
    )
)
WITH CHECK (
    public.is_super_admin()
    OR (public.current_user_role() <> 'STAFF'::app_role AND created_by = auth.uid())
    OR (public.current_user_role() = 'STAFF'::app_role AND public.has_staff_permission('manage_assignments'))
);
...
USING (
    ...
    OR (
        ...
        AND (matter_id IS NULL OR public.staff_can_access_matter(matter_id, 'view_matters'))
    )
);
...
WITH CHECK (
    public.is_super_admin()
    OR (public.current_user_role() <> 'STAFF'::app_role AND created_by = auth.uid())
    OR (public.current_user_role() = 'STAFF'::app_role AND public.has_staff_permission('assign_matters') AND created_by = auth.uid())
);
...
USING (
    public.is_super_admin()
    OR created_by = auth.uid()
    OR (public.current_user_role() = 'STAFF'::app_role AND public.has_staff_permission('assign_matters') AND assigned_staff_id IN (SELECT s.id FROM public.staff s WHERE s.user_id = auth.uid() AND s.is_active = true))
)
WITH CHECK (
    public.is_super_admin()
    OR created_by = auth.uid()
    OR (public.current_user_role() = 'STAFF'::app_role AND public.has_staff_permission('assign_matters'))
);
...
USING (
    ...
    OR individual_user_id = auth.uid()
    OR business_id IN (SELECT b.id FROM public.businesses b WHERE b.owner_user_id = auth.uid())
    OR (public.current_user_role() = 'STAFF'::app_role AND public.has_staff_permission('view_financials') AND matter_id IS NOT NULL AND public.staff_can_access_matter(matter_id, 'view_financials'))
);
...
WITH CHECK (
    public.is_super_admin()
    OR (public.current_user_role() <> 'STAFF'::app_role AND created_by = auth.uid())
    OR (public.current_user_role() = 'STAFF'::app_role AND public.has_staff_permission('manage_invoices') AND created_by = auth.uid())
);
...
USING (
    public.is_super_admin()
    OR (public.current_user_role() <> 'STAFF'::app_role AND created_by = auth.uid())
    OR (public.current_user_role() = 'STAFF'::app_role AND public.has_staff_permission('manage_invoices') AND (created_by = auth.uid() OR (matter_id IS NOT NULL AND public.staff_can_access_matter(matter_id, 'manage_invoices'))))
)
WITH CHECK (
    public.is_super_admin()
    OR (public.current_user_role() <> 'STAFF'::app_role AND created_by = auth.uid())
    OR (public.current_user_role() = 'STAFF'::app_role AND public.has_staff_permission('manage_invoices'))
);
...
USING (
    ...
    OR EXISTS (
        ...
        WHERE i.id = payments.invoice_id
          AND (i.individual_user_id = auth.uid() OR i.business_id IN (SELECT b.id FROM public.businesses b WHERE b.owner_user_id = auth.uid())
               OR (public.current_user_role() = 'STAFF'::app_role AND public.has_staff_permission('view_financials') AND i.matter_id IS NOT NULL AND public.staff_can_access_matter(i.matter_id, 'view_financials')))
    )
);
...
WITH CHECK (
    public.is_super_admin()
    OR (public.current_user_role() <> 'STAFF'::app_role AND received_by = auth.uid())
    OR (public.current_user_role() = 'STAFF'::app_role AND public.has_staff_permission('manage_invoices') AND received_by = auth.uid())
);
...
USING (
    public.is_super_admin()
    OR received_by = auth.uid()
    OR (public.current_user_role() = 'STAFF'::app_role AND public.has_staff_permission('manage_invoices') AND EXISTS (SELECT 1 FROM public.invoices i WHERE i.id = payments.invoice_id AND i.matter_id IS NOT NULL AND public.staff_can_access_matter(i.matter_id, 'manage_invoices')))
)
WITH CHECK (
    public.is_super_admin()
    OR received_by = auth.uid()
