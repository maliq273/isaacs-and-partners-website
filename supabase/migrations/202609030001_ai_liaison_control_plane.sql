...
-- Super Admin always supersedes staff permissions and is included in human
-- intervention routing. No provider credentials are stored here.
...
create table if not exists public.ai_conversations (
    ...
    client_user_id uuid null references public.profiles(id) on delete set null,
...
create unique index if not exists ai_conversations_chat_uidx
    on public.ai_conversations(chat_id) where chat_id is not null;
create index if not exists ai_conversations_client_idx
    on public.ai_conversations(client_user_id, updated_at desc);
create index if not exists ai_conversations_matter_idx
    on public.ai_conversations(matter_id, updated_at desc);
...
-- -----------------------------------------------------------------------------
-- Explicit AI authority matrix.
-- This is deliberately separate from general staff permissions because the
-- ability to liaise with an AI agent is a distinct business authority.
-- -----------------------------------------------------------------------------
...
create index if not exists staff_ai_permissions_active_idx
    on public.staff_ai_permissions(staff_user_id, is_active);
...
create index if not exists staff_ai_permission_audit_staff_idx
    on public.staff_ai_permission_audit(staff_user_id, changed_at desc);

-- -----------------------------------------------------------------------------
-- Human intervention queue.
-- Each intervention is assigned to an authorised staff member where possible,
-- while Super Admin review remains mandatory.
-- -----------------------------------------------------------------------------
...
create index if not exists human_interventions_status_idx
    on public.human_interventions(status, priority, created_at);
create index if not exists human_interventions_staff_idx
    on public.human_interventions(assigned_staff_id, status, updated_at desc);
create index if not exists human_interventions_client_idx
    on public.human_interventions(client_user_id, created_at desc);
create index if not exists human_interventions_conversation_idx
    on public.human_interventions(conversation_id, created_at desc);
...
create index if not exists ai_agent_events_conversation_idx
    on public.ai_agent_events(conversation_id, created_at desc);
create index if not exists ai_agent_events_matter_idx
    on public.ai_agent_events(matter_id, created_at desc);
...
for each row execute function public.set_ai_liaison_updated_at();

drop trigger if exists staff_ai_permissions_updated_at on public.staff_ai_permissions;
create trigger staff_ai_permissions_updated_at
before update on public.staff_ai_permissions
for each row execute function public.set_ai_liaison_updated_at();

drop trigger if exists human_interventions_updated_at on public.human_interventions;
create trigger human_interventions_updated_at
before update on public.human_interventions
for each row execute function public.set_ai_liaison_updated_at();

-- -----------------------------------------------------------------------------
-- AI capability helper.
-- Super Admin is always authoritative and bypasses the staff matrix.
-- -----------------------------------------------------------------------------
...
-- Backward-safe fallback where the project only exposes is_super_admin().
-- The canonical helper above intentionally calls the user-scoped helper when
-- available; this block creates it if the database does not already have it.
do $$
begin
    if to_regprocedure('public.is_super_admin_for_user(uuid)') is null then
        execute $fn$
            create function public.is_super_admin_for_user(p_user_id uuid)
            returns boolean
            language sql
            security definer
            set search_path = public
            ...
            as $body$
                select exists (
                    ...
                    where p.id = p_user_id
                      and (
                          upper(coalesce(p.role::text, '')) = 'SUPER_ADMIN'
                          or coalesce((p.metadata->>'is_super_admin')::boolean, false) = true
                      )
                );
            $body$;
        $fn$;
...
-- Re-create after fallback helper resolution so the function is valid on both
-- existing and clean environments.
create or replace function public.staff_ai_can(p_staff_user_id uuid, p_capability text)
returns boolean
language sql
security definer
...
create policy ai_conversations_client_select on public.ai_conversations
...
using (
    ...
    or (
        ...
        and (matter_id is null or public.staff_can_access_matter(matter_id, 'view_communications'))
    )
);

drop policy if exists ai_conversations_client_insert on public.ai_conversations;
create policy ai_conversations_client_insert on public.ai_conversations
for insert to authenticated
with check (client_user_id = auth.uid());

drop policy if exists ai_conversations_staff_update on public.ai_conversations;
create policy ai_conversations_staff_update on public.ai_conversations
for update to authenticated
using (
    public.is_super_admin()
    or (
        public.current_user_role() = 'STAFF'::app_role
        and public.has_staff_permission('manage_communications')
        and (matter_id is null or public.staff_can_access_matter(matter_id, 'manage_communications'))
    )
)
with check (
    public.is_super_admin()
...
create policy ai_messages_authorised_select on public.ai_conversation_messages
for select to authenticated
using (
    public.is_super_admin()
    or exists (
...
create policy staff_ai_permissions_self_select on public.staff_ai_permissions
for select to authenticated
...
create policy staff_ai_permissions_super_admin_write on public.staff_ai_permissions
for all to authenticated
using (public.is_super_admin())
with check (public.is_super_admin());

drop policy if exists staff_ai_permission_audit_super_admin_select on public.staff_ai_permission_audit;
create policy staff_ai_permission_audit_super_admin_select on public.staff_ai_permission_audit
for select to authenticated
...
create policy human_interventions_authorised_select on public.human_interventions
for select to authenticated
using (
    public.is_super_admin()
...
create policy human_interventions_authorised_update on public.human_interventions
for update to authenticated
using (
    public.is_super_admin()
    or assigned_staff_id = auth.uid()
    or (
        public.current_user_role() = 'STAFF'::app_role
        and public.staff_ai_can(auth.uid(), 'ANSWER_AI_QUERIES')
...
with check (
    public.is_super_admin()
    or assigned_staff_id = auth.uid()
    or public.staff_ai_can(auth.uid(), 'ANSWER_AI_QUERIES')
);

drop policy if exists ai_agent_events_authorised_select on public.ai_agent_events;
create policy ai_agent_events_authorised_select on public.ai_agent_events
for select to authenticated
using (
    public.is_super_admin()
...
-- Client messages/events are read-only from the browser. Server-side workers
-- use service_role for writes, keeping the browser outside the AI event ledger.
...
grant select on public.staff_ai_permissions to authenticated;
grant select on public.staff_ai_permission_audit to authenticated;
grant select on public.human_interventions to authenticated;

-- -----------------------------------------------------------------------------
-- Permission audit trigger. Only Super Admin may change AI authority, and every
-- changed boolean is recorded independently for an auditable history.
-- -----------------------------------------------------------------------------
...
begin
    ...
    ) as x(capability, old_value, new_value)
...
create trigger staff_ai_permission_audit_trigger
before insert or update on public.staff_ai_permissions
for each row execute function public.audit_staff_ai_permission_changes();

-- -----------------------------------------------------------------------------
-- Secure RPC for creating a human intervention. This guarantees that the AI
-- can never silently omit Super Admin from a required human escalation.
-- -----------------------------------------------------------------------------
...
begin
    ...
    select c.client_user_id, c.matter_id
      into v_client, v_matter
    ...
    if v_matter is null then
        v_matter := p_matter_id;
    end if;

    if not public.is_super_admin() then
        if auth.uid() <> coalesce(v_client, auth.uid())
           and not public.staff_ai_can(auth.uid(), 'LIAISE_WITH_AI') then
            raise exception 'AI liaison authority required.' using errcode = '42501';
        end if;
    end if;

    if p_assigned_staff_id is not null then
        if not public.staff_ai_can(p_assigned_staff_id, 'LIAISE_WITH_AI')
           or not public.staff_ai_can(p_assigned_staff_id, 'ANSWER_AI_QUERIES') then
            raise exception 'Assigned staff member is not authorised to answer AI queries.' using errcode = '42501';
        end if;
        v_staff := p_assigned_staff_id;
    end if;

    insert into public.human_interventions (
        conversation_id, matter_id, client_user_id, assigned_staff_id,
        super_admin_required, status, reason, priority, question, ai_context
    ) values (
        p_conversation_id, v_matter, v_client, v_staff,
        true,
        case when v_staff is null then 'PENDING' else 'ASSIGNED' end,
        nullif(trim(p_reason), ''),
        upper(coalesce(p_priority, 'NORMAL')),
        nullif(trim(p_question), ''),
        coalesce(p_ai_context, '{}'::jsonb)
    ) returning id into v_id;

    update public.ai_conversations
       set state = 'AI_ESCALATED',
           updated_at = now()
...
comment on table public.staff_ai_permissions is 'Super Admin-controlled AI liaison authority matrix. Separate from general staff permissions.';
comment on table public.staff_ai_permission_audit is 'Immutable audit trail for Super Admin changes to AI liaison authority.';
comment on table public.human_interventions is 'Human escalation queue. Super Admin review is mandatory for every intervention.';
comment on table public.ai_conversations is 'Authoritative AI conversation state and handover state.';
