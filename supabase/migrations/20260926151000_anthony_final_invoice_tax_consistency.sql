-- Keep final-balance invoice totals internally consistent: the final 50% is treated as the
-- already-approved balance amount, so no additional tax is silently added.
begin;
...
begin
  ...
  select id into v_invoice_id from public.invoices where matter_id=p_matter_id and payment_stage='FINAL_BALANCE'
    order by created_at desc limit 1;
  if v_invoice_id is not null then
    update public.anthony_workflow_state set state='FINAL_PAYMENT_PENDING',
      application_completed_at=coalesce(application_completed_at,now()),release_status='HELD_PENDING_FINAL_PAYMENT',updated_at=now()
  ...
  if v_balance <= 0 then
    update public.anthony_workflow_state set state='SUBMISSION_RELEASED',
      application_completed_at=coalesce(application_completed_at,now()),final_payment_verified_at=now(),
      submission_released_at=now(),release_status='RELEASED_AFTER_FINAL_PAYMENT',updated_at=now()
      where matter_id=p_matter_id;
    return v_source.id;
  end if;
  v_number:='INV-FINAL-'||to_char(current_date,'YYYYMMDD')||'-'||upper(substr(replace(gen_random_uuid()::text,'-',''),1,8));
  insert into public.invoices(
    matter_id,quote_id,business_id,individual_user_id,invoice_number,invoice_date,due_date,terms,currency,subject,
    subtotal,discount_type,discount_value,discount_amount,tax_rate,tax_amount,shipping_charge,adjustment,total,
    amount_paid,balance_due,status,customer_notes,terms_and_conditions,created_by,payment_stage,parent_invoice_id
  ) values (
    p_matter_id,v_source.quote_id,v_source.business_id,v_source.individual_user_id,v_number,current_date,current_date,
    'DUE_ON_RECEIPT',coalesce(v_source.currency,'ZAR'),'Final 50% balance — application dossier completed',
    v_balance,'PERCENT',0,0,0,0,0,0,v_balance,0,v_balance,'SENT',
    'Final 50% payment is required before release for DHA/VFS submission.',
    coalesce(v_source.terms_and_conditions,v_source.terms),coalesce(v_source.created_by,auth.uid()),'FINAL_BALANCE',v_source.id
  ...
  values(v_invoice_id,1,'Final 50% Professional Fee',
    'Final balance due when the application dossier is complete, before DHA/VFS submission.',1,v_balance,0,'PERCENT',0,v_balance);
  ...
  update public.client_documents set commercial_release_status='HELD_PENDING_FINAL_PAYMENT',updated_at=now() where matter_id=p_matter_id;
  return v_invoice_id;
end;
$$;
