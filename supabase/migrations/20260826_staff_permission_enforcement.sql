...
-- Apply in Supabase SQL Editor after reviewing against the live schema.
...
create or replace function public.staff_can_access_matter(
    p_matter_id uuid,
...
as $$
    with access as (
        select public.staff_permission_scope(p_permission_key) as scope
    )
    select
        public.is_super_admin()
        or exists (
              ...
              and exists (
                  ...
                  where m.id = p_matter_id
                    and (
                        ...
                        or m.individual_user_id = auth.uid()
                        or exists (
                            select 1
...
create or replace function public.staff_can_access_case(
    p_case_id uuid,
    p_permission_key text
)
returns boolean
    ...
    select
        ...
        or exists (
            ...
            where c.id = p_case_id
              and public.staff_can_access_matter(
                    c.matter_id,
                    p_permission_key
                  )
        );
...
create or replace function public.staff_can_access_quote(
    p_quote_id uuid,
    p_permission_key text
)
returns boolean
        ...
        or exists (
            ...
            where q.id = p_quote_id
              and (
                  ...
                  or (
                      public.staff_permission_scope(p_permission_key) = 'ASSIGNED'
                      and exists (
                          select 1
                          from public.assignments a
                          join public.staff s on s.id = a.staff_id
                  ...
                  or (
                      public.staff_permission_scope(p_permission_key) = 'DEPARTMENT'
...
create or replace function public.staff_can_access_assignment(
...
create policy matters_select_owner_staff_admin
on public.matters
for select
to authenticated
using (
...
create policy matters_admin_update
on public.matters
for update
to authenticated
using (
...
create policy cases_select_authorised
on public.cases
for select
to authenticated
using (
...
create policy quotes_select_authorised
on public.quotes
for select
to authenticated
using (
...
-- Staff must have the explicit permission before creating/updating assignments.
...
comment on function public.staff_permission_scope(text)
is 'Returns the strongest enabled permission scope for the authenticated staff member.';

comment on function public.has_staff_permission(text)
is 'Returns true only for SUPER_ADMIN or an active staff account with the requested enabled permission.';
