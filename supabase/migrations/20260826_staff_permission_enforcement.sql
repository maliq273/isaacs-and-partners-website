-- Isaacs & Partners
-- Staff permission enforcement migration.
...
returns boolean language sql security definer set search_path = public stable
as $$ with access as (select public.staff_permission_scope(p_permission_key) scope) select public.is_super_admin() or exists(select 1 from access where scope='ALL') or exists(select 1 from access where scope='OWN' and exists(select 1 from public.matters m where m.id=p_matter_id and (m.created_by=auth.uid() or m.individual_user_id=auth.uid() or exists(select 1 from public.businesses b where b.id=m.business_id and b.owner_user_id=auth.uid())))) or exists(select 1 from access where scope='ASSIGNED' and exists(select 1 from public.assignments a join public.staff s on s.id=a.staff_id where a.matter_id=p_matter_id and s.user_id=auth.uid() and s.is_active=true and a.status='ACTIVE'::assignment_status)) or exists(select 1 from access where scope='DEPARTMENT' and exists(select 1 from public.assignments a join public.staff assigned_staff on assigned_staff.id=a.staff_id join public.staff current_staff on current_staff.user_id=auth.uid() where a.matter_id=p_matter_id and a.status='ACTIVE'::assignment_status and assigned_staff.department=current_staff.department and current_staff.is_active=true)); $$;
