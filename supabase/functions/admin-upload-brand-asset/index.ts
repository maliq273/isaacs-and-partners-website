...
const extensionFor = (mime: string) => ({ "image/jpeg": "jpg", "image/png": "png", "image/webp": "webp" }[mime] ?? null);
...
Deno.serve(async request => {
  ...
  if (!supabaseUrl || !serviceRoleKey || !anonKey || !token) return json({ error: "Asset service authentication is not fully configured." }, 500);
  ...
  const { data: githubConfig, error: configError } = await admin.rpc("get_github_integration_secret");
  if (configError) return json({ error: `GitHub integration could not be loaded: ${configError.message}` }, 500);
  const config = Array.isArray(githubConfig) ? githubConfig[0] : githubConfig;
  ...
  if (existingResponse.ok) existingSha = (await existingResponse.json()).sha;
  else if (existingResponse.status !== 404) return json({ error: `GitHub asset lookup failed: ${await existingResponse.text()}` }, 502);

  const githubResponse = await fetch(apiBase, {
    method: "PUT",
    headers,
    body: JSON.stringify({ message: `chore: update ${role.toLowerCase()} brand asset ${userId}`, content: base64, ...(existingSha ? { sha: existingSha } : {}) })
  });
