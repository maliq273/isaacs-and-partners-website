...
const TEMPLATES:Record<string,string>={DHA_84:"immigrations_docs/dha84-form11.pdf",DHA_1738:"immigrations_docs/FORM-DHA-1738_e-version.pdf",BI_947:"immigrations_docs/BI-947.pdf",BI_1712A:"immigrations_docs/Spousal-RelationshipBI-1712A-Form-12-1.pdf"};
...
if(fields.length===0){
 ...
 if(unresolved.length)throw new Error("The application is not complete enough to generate this official form. Resolve the outstanding coordinate fields/answers first.");
} else {
 form.updateFieldAppearances(); form.flatten();
}
const bytes=await pdf.save();const path=`applications/${matterId||"unassigned"}/${crypto.randomUUID()}-${templatePath(template).split("/").pop()}`;const upload=await admin.storage.from("immigration-generated").upload(path,bytes,{contentType:"application/pdf",cacheControl:"3600",upsert:false});if(upload.error)throw upload.error;const record=await admin.from("immigration_application_documents").insert({matter_id:matterId||null,client_user_id:clientUserId||null,case_type:metadata?.caseType||"unknown",template_path:loaded.path,template_sha256:loaded.hash,generated_path:path,status:"NEEDS_REVIEW",field_manifest:fields.length?applied:coordinateApplied,answers_snapshot:answers||{},checklist:metadata?.checklist||[],provenance:{source:"Anthony Isaacs",template_source:"GitHub /immigrations_docs",generated_by:actor||"SYSTEM"},generated_by:actor||null,generated_at:new Date().toISOString()}).select("id").single();if(record.error)throw record.error;const signed=await admin.storage.from("immigration-generated").createSignedUrl(path,604800);if(signed.error)throw signed.error;return {ok:true,documentId:record.data.id,template:loaded.path,templateSha256:loaded.hash,generatedPath:path,signedUrl:signed.data.signedUrl,appliedFields:fields.length?applied:coordinateApplied,coordinateMapUsed:fields.length===0,reviewStatus:"NEEDS_REVIEW",submissionReady:false};}
