...
Deno.serve(async request => {
  ...
  if (request.method === "GET") {
    const { data, error } = await admin
      ...
      .select("repository,configured_at,last_tested_at,last_test_status,last_test_message")
      .eq("id", "default")
      .maybeSingle();
    if (error) return json({ error: error.message }, 500);
    return json({
      configured: Boolean(data?.configured_at),
      repository: data?.repository ?? "maliq273/isaacs-and-partners-website",
  ...
  if (action === "save") {
    ...
    const { data, error } = await admin.rpc("set_github_integration_config", {
    ...
    await admin.from("audit_logs").insert({
      ...
      action: "GITHUB_INTEGRATION_CONFIGURED",
      entity_type: "github_integration",
      entity_id: "default",
      new_data: { repository, token_updated: true },
      metadata: { source: "admin-github-config" }
    });

    return json({ success: true, ...(data ?? {}), token_masked: `${githubToken.slice(0, 4)}••••••••${githubToken.slice(-4)}` });
  }
  ...
  if (action === "test") {
    ...
    if (!config?.github_token || !config?.repository) {
      ...
      return json({ error: "GitHub integration is not configured." }, 400);
    }
    ...
    if (!response.ok) {
      ...
      await admin.rpc("record_github_integration_test", { p_status: "FAIL", p_message: message });
    ...
    await admin.from("audit_logs").insert({
      actor_user_id: caller.user.id,
      action: "GITHUB_INTEGRATION_TESTED",
      entity_type: "github_integration",
      entity_id: "default",
      new_data: { repository: config.repository, success: canPush, permissions: { push: Boolean(permissions.push), maintain: Boolean(permissions.maintain), admin: Boolean(permissions.admin) } },
      metadata: { source: "admin-github-config" }
    });
