 ...
 *
 * Authenticated AI runtime for the client portal and trusted WhatsApp bridge.
 * The browser never receives OpenWA credentials.
 */
...
const corsHeaders = {
  ...
  "Access-Control-Allow-Headers": "authorization, x-client-info, apikey, content-type, x-ai-internal-worker-token",
...
function normaliseChannel(value: unknown) {
  ...
  if (!["WHATSAPP", "PORTAL", "EMAIL", "OTHER"].includes(channel)) throw new Error("Invalid conversation channel.");
  return channel;
}
...
async function authenticate(request: Request, payload: any) {
  ...
  if (internalToken) {
    if (!INTERNAL_WORKER_TOKEN || internalToken !== INTERNAL_WORKER_TOKEN) {
      throw new Response(JSON.stringify({ error: "Invalid internal worker credentials." }), { status: 401, headers: corsHeaders });
    }
...
async function verifyMatterOwnership(userId: string, matterId: string | null) {
  ...
  if (matter.business_id) {
    const { data: business, error } = await admin.from("businesses").select("id").eq("id", matter.business_id).eq("owner_user_id", userId).maybeSingle();
    if (error) throw error;
...
async function resolveConversation({ userId, chatId, phoneNumber, channel, matterId }: { userId: string; chatId: string | null; phoneNumber: string | null; channel: string; matterId: string | null }) {
  let query = admin.from("ai_conversations").select("*").eq("client_user_id", userId).eq("channel", channel).order("updated_at", { ascending: false }).limit(1);
  if (chatId) query = query.eq("chat_id", chatId);
  const { data: existing, error } = await query.maybeSingle();
  if (error) throw error;
  if (existing) return existing;

  const { data: created, error: createError } = await admin.from("ai_conversations").insert({
...
async function persistAiMessage({ conversationId, body, intent, serviceDomain, metadata = {} }: { conversationId: string; body: string; intent?: string | null; serviceDomain?: string | null; metadata?: Record<string, unknown> }) {
  const { data, error } = await admin.rpc("ai_append_conversation_message", {
    ...
    p_metadata: { source: "ai-liaison-runtime", runtime: "WhatsAppAgent", ...metadata },
  });
  if (error) throw error;
  return data;
}

async function queueWhatsAppAiReply({ userId, conversation, body, phoneNumber, matterId, sourceMessageId }: { userId: string; conversation: any; body: string; phoneNumber: string | null; matterId: string | null; sourceMessageId: string | null }) {
  if (conversation.channel !== "WHATSAPP") return null;
  if (!conversation.chat_id) throw new Error("WhatsApp conversation is missing chatId.");

  const idempotencyKey = sourceMessageId ? `ai-reply:${conversation.id}:${sourceMessageId}` : `ai-reply:${conversation.id}:${Date.now()}`;
  const { data: existing } = await admin.from("communication_messages").select("id").eq("idempotency_key", idempotencyKey).maybeSingle();
  if (existing?.id) return existing.id;

  const { data: message, error: messageError } = await admin.from("communication_messages").insert({
    customer_user_id: userId,
    matter_id: matterId || conversation.matter_id || null,
    channel: "WHATSAPP",
    direction: "OUTBOUND",
    phone_number: phoneNumber || conversation.phone_number || null,
  ...
  const { error: outboxError } = await admin.from("communication_outbox").insert({ message_id: message.id, session_id: null, chat_id: conversation.chat_id, status: "QUEUED" });
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (channel === "WHATSAPP" && !chatId) return json({ error: "WhatsApp conversations require chatId." }, 400);
    ...
    const { data: message, error: messageError } = await caller.rpc("ai_append_conversation_message", {
      ...
      p_metadata: { source: "ai-liaison-runtime", ...(messageId ? { client_message_id: messageId } : {}), transport: internal ? "openwa" : "portal" },
    });
    if (messageError) throw messageError;

    const agent = createLiaisonAgent();
    const result = await agent.handleInbound({ chatId: chatId || `portal:${user.id}`, phoneNumber, body, messageId, user, matter, conversation: null });
    ...
    if (aiReply) {
      aiMessage = await persistAiMessage({
        ...
        metadata: {
          ...
          ...(result?.escalation ? { escalation: { superAdminRequired: result.escalation.superAdminRequired === true, requiredCapabilities: result.escalation.requiredCapabilities ?? [] } } : {}),
        },
      });

      if (channel === "WHATSAPP" && result?.action === "RESPOND") {
        transportMessageId = await queueWhatsAppAiReply({ userId: user.id, conversation, body: aiReply, phoneNumber, matterId, sourceMessageId: messageId });
      }
    }

    if (result?.action === "ESCALATE") {
      const { error } = await admin.from("ai_conversations").update({ state: "AI_ESCALATED", updated_at: new Date().toISOString() }).eq("id", conversation.id);
      if (error) throw error;
    }
    ...
    return json({
      ...
      next: {
        ...
        whatsappTransport: transportMessageId ? "QUEUED_FOR_OPENWA" : "NONE",
