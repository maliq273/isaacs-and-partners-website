...
class DashboardPageController{
  ...
  renderSuperAdmin(){
    ...
    this.setText("#admin-watchdog-open",c.openWatchdog??0);this.setText("#admin-appointments-today",c.appointmentsToday??0);this.setText("#admin-documents-outstanding",c.outstandingDocuments??0);this.setText("#admin-invoices-outstanding",c.outstandingInvoices??0);this.setText("#admin-pending-quotes",c.pendingPreQuotes??0);
    this.setText("#admin-people-meta",`${c.activeStaff??0} active staff record(s)`);this.setText("#admin-open-matters-meta",`${c.openMatters??0} active matter(s)`);this.setText("#admin-unassigned-meta",`${c.unassignedMatters??0} require assignment`);this.setText("#admin-watchdog-meta",c.openWatchdog?"Requires response attention":"No open response obligations");this.setText("#admin-invoice-meta",money(c.outstandingBalance));
    ...
    this.renderNav();this.renderOverview();this.renderAuthority();this.renderCommunication();this.renderOperations();this.renderFinance();this.renderNotifications();this.renderIntegrations();this.renderWatchdog();
  }
  ...
  renderCommunication(){const c=this.data.counts||{},root=document.querySelector("#admin-communication-metrics");if(root)root.innerHTML=[["Active WhatsApp contacts",c.activeContacts||0],["Inbound today",c.inboundToday||0],["Outbound today",c.outboundToday||0],["Open response obligations",c.openWatchdog||0],["Unread/open notifications",c.pendingNotifications||0]].map(x=>`<div><span>${esc(x[0])}</span><strong>${esc(x[1])}</strong></div>`).join("")}
  ...
  renderWatchdog(){const root=document.querySelector("#admin-watchdog-list");if(!root)return;const rows=[...(this.data.watchdog||[])].filter(x=>["OPEN","WAITING_SUPER_ADMIN"].includes(String(x.status||"").toUpperCase())).sort((a,b)=>new Date(a.next_due_at)-new Date(b.next_due_at)).slice(0,8);root.innerHTML=rows.length?rows.map(w=>`<div class="admin-alert-row"><div><strong>${esc(w.phone_number||w.chat_id||"WhatsApp contact")}</strong><small>${esc(w.status)} · next action ${esc(formatDate(w.next_due_at))}</small></div><span class="status-badge ${w.status==="WAITING_SUPER_ADMIN"?"pending":"inactive"}">${esc(w.reminder_count||0)} reminders</span></div>`).join(""):"<div class='empty-state'>No open WhatsApp response obligations.</div>"}
