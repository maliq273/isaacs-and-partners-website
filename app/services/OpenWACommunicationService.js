 ...
 *
 * Browser-safe facade. It only queues messages through the authenticated RPC
...
export default class OpenWACommunicationService {
    ...
    async listMessages({ matterId = null, limit = 100 } = {}) {
        let query = this.supabase.from("communication_messages").select("*").order("created_at", { ascending: false }).limit(Math.min(Math.max(Number(limit) || 100, 1), 200));
