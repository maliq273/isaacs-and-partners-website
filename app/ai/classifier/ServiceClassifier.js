...
function distance(a, b) { const left = String(a); const right = String(b); const row = Array.from({ length: right.length + 1 }, (_, i) => i); for (let i = 1; i <= left.length; i++) { let previous = row[0]; row[0] = i; for (let j = 1; j <= right.length; j++) { const current = row[j]; row[j] = Math.min(row[j] + 1, row[j - 1] + 1, previous + (left[i - 1] === right[j - 1] ? 0 : 1)); previous = current; } } return row[right.length]; }
    ...
    classify(input = {}) {
        ...
        for (const word of words) {
            for (const service of candidates) {
                for (const token of service.name.toLowerCase().split(/\s+/)) {
                    if (word === token) continue;
                    if (distance(word, token) <= (token.length >= 7 ? 2 : 1)) return { value: service.category, confidence: 0.88, matched: [service.name], serviceId: service.id, serviceName: service.name, correctedInput: { from: word, to: token } };
                }
            }
        }
