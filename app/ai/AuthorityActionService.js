...
export default class AuthorityActionService {
  ...
  canExecute(identity, permission) {
    ...
    if (isOwnerCreator(identity)) return true;
  ...
  async executePermissionMutation({ identity, classification, message, conversationId }) {
    ...
    await this.audit({ authorityId, actorId, actorPhone: identity?.sourcePhone, action: classification.action, targetType: "AUTHORITY", targetId: row.id, requestText: message, decision: "EXECUTED", reason: isOwnerCreator(identity) ? "Owner/creator-authorised permission mutation." : "Authorised permission mutation.", beforeData: before, afterData: saved.data.action_permissions, metadata: { conversation_id: conversationId, capability: classification.capability, grant: classification.grant === true, owner_creator: isOwnerCreator(identity) } });
  ...
  async executeAuthorityMutation({ identity, classification, message, conversationId }) {
    ...
    await this.audit({ authorityId, actorId, actorPhone: identity?.sourcePhone, action: classification.action, targetType: "AUTHORITY", targetId: saved.data.id, requestText: message, decision: "EXECUTED", reason: isOwnerCreator(identity) ? "Owner/creator-authorised authority mutation." : "Authorised authority-directory mutation.", beforeData: existing, afterData: saved.data, metadata: { conversation_id: conversationId, owner_creator: isOwnerCreator(identity) } });
