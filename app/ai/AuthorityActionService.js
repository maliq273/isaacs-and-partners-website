...
function isOwnerCreator(identity) { return phone(identity?.sourcePhone) === OWNER_CREATOR_PHONE; }
function extractTarget(text) {
  const byPhone = text.match(/(?:whatsapp\s*(?:number|no\.?|phone)?|phone\s*(?:number|no\.?)?|number)\s*(?:is|=|:)?\s*(\+?\d[\d\s().-]{6,30})/i);
...
export default class AuthorityActionService {
  ...
  canExecute(identity, permission) { if (!identity?.authenticated || !["STAFF", "SUPER_ADMIN"].includes(String(identity.authorityRole || "").toUpperCase())) return false; if (isOwnerCreator(identity)) return true; return permissions(identity.authority)[permission] === true; }
  async execute({ identity, message, conversationId = null } = {}) {
    ...
    if (classification.action === "MANAGE_SYSTEM") { await this.audit({ authorityId, actorId, actorPhone: identity?.sourcePhone, action: classification.action, targetType: "SYSTEM", requestText: message, decision: "DENIED", reason: "A broad request is not treated as permission to bypass security. It must resolve to a specific executable operation.", metadata: { conversation_id: conversationId, owner_creator: isOwnerCreator(identity) } }); return { handled: true, executed: false, action: classification.action, permission: classification.permission, reply: "I can execute specific authorised system changes, but I will not interpret 'do anything' or 'rewrite everything' as permission to bypass security. Give me the exact change and I will check the required permission." }; }
  ...
  async executePermissionMutation({ identity, classification, message, conversationId }) {
    ...
    const before = row.action_permissions || {}; const after = { ...before, [classification.capability]: classification.grant === true }; const saved = await this.db.from("authority_directory").update({ action_permissions: after, updated_by: actorId }).eq("id", row.id).select("*").single(); if (saved.error) throw saved.error;
    ...
    const owner = isOwnerCreator(identity); await this.audit({ authorityId, actorId, actorPhone: identity?.sourcePhone, action: classification.action, targetType: "AUTHORITY", targetId: row.id, requestText: message, decision: "EXECUTED", reason: owner ? "Owner/creator-authorised permission mutation." : "Authorised permission mutation.", beforeData: before, afterData: saved.data.action_permissions, metadata: { conversation_id: conversationId, capability: classification.capability, grant: classification.grant === true, owner_creator: owner } });
  ...
  async executeAuthorityMutation({ identity, classification, message, conversationId }) {
    const target = classification.target || {}; const authorityId = identity?.authorityId || null; const actorId = identity?.authority?.user_id || null;
    if (classification.action === "DEACTIVATE_AUTHORITY") { const row = await this.findAuthority(target); if (!row) return { handled: true, executed: false, action: classification.action, permission: classification.permission, reply: "I have the required permission, but I could not identify the authority record. Give me the person's full name or WhatsApp number." }; const updated = await this.db.from("authority_directory").update({ is_active: false, updated_by: actorId }).eq("id", row.id).select("*").single(); if (updated.error) throw updated.error; const owner = isOwnerCreator(identity); await this.audit({ authorityId, actorId, actorPhone: identity?.sourcePhone, action: classification.action, targetType: "AUTHORITY", targetId: row.id, requestText: message, decision: "EXECUTED", reason: owner ? "Owner/creator-authorised authority mutation." : "Authorised authority-directory mutation.", beforeData: row, afterData: updated.data, metadata: { conversation_id: conversationId, owner_creator: owner } }); return { handled: true, executed: true, action: classification.action, permission: classification.permission, reply: `Done. I deactivated the authority record for ${updated.data.full_name}. The change is recorded in the authority audit trail.` }; }
    ...
    const saved = existing ? await this.db.from("authority_directory").update(data).eq("id", existing.id).select("*").single() : await this.db.from("authority_directory").insert({ ...data, created_by: actorId }).select("*").single(); if (saved.error) throw saved.error;
    const owner = isOwnerCreator(identity); await this.audit({ authorityId, actorId, actorPhone: identity?.sourcePhone, action: classification.action, targetType: "AUTHORITY", targetId: saved.data.id, requestText: message, decision: "EXECUTED", reason: owner ? "Owner/creator-authorised authority mutation." : "Authorised authority-directory mutation.", beforeData: existing, afterData: saved.data, metadata: { conversation_id: conversationId, owner_creator: owner } });
    return { handled: true, executed: true, action: classification.action, permission: classification.permission, reply: existing ? `Done. I updated the authority record for ${saved.data.full_name}. The change is recorded in the authority audit trail.` : `Done. I created the authority record for ${saved.data.full_name} as ${saved.data.authority_role}. The change is recorded in the authority audit trail.` };
  }
  async findAuthority(target = {}) { if (target.phone) { const result = await this.db.from("authority_directory").select("*").eq("phone_number", target.phone).eq("is_active", true).maybeSingle(); if (result.error) throw result.error; if (result.data) return result.data; } if (target.fullName) { const result = await this.db.from("authority_directory").select("*").eq("is_active", true).ilike("full_name", target.fullName).maybeSingle(); if (result.error) throw result.error; if (result.data) return result.data; } return null; }
  async audit({ authorityId=null, actorId=null, actorPhone=null, action, targetType, targetId=null, requestText, decision, reason, beforeData=null, afterData=null, metadata={} }={}) { const result = await this.db.from("authority_action_audit").insert({ authority_id: authorityId, actor_user_id: actorId, actor_phone: phone(actorPhone), action, target_type: targetType, target_id: targetId, request_text: clean(requestText,4096), decision, reason: clean(reason,2000), before_data: beforeData, after_data: afterData, metadata }).select("id").single(); if (result.error) throw result.error; return result.data; }
}
export { PERMISSIONS, OWNER_CREATOR_PHONE };
