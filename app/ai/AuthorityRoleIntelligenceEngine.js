...
const ACTION_TO_CAPABILITY = Object.freeze({
  ...
  CREATE_AUTHORITY:ACTION_CAPABILITIES.MANAGE_AUTHORITY,
  UPDATE_AUTHORITY:ACTION_CAPABILITIES.MANAGE_AUTHORITY,
  DEACTIVATE_AUTHORITY:ACTION_CAPABILITIES.MANAGE_AUTHORITY,
  MANAGE_STAFF_PERMISSIONS:ACTION_CAPABILITIES.MANAGE_STAFF,
...
export default class AuthorityRoleIntelligenceEngine {
  ...
  async resolveWhatsAppIdentity({phoneNumber,chatId=null,whatsappName=null}={}){
    ...
    return{
      ...
      permissionsRecordActive:Boolean(permissions?.is_active===true||Object.keys(actionPermissions).length>0),
      displayNameMatches
    };
  }
  ...
  _requiredCapability(action,domain){
    ...
    return{IMMIGRATION:ACTION_CAPABILITIES.HANDLE_IMMIGRATION,HR:ACTION_CAPABILITIES.HANDLE_HR,HR_IR:ACTION_CAPABILITIES.HANDLE_HR,BUSINESS_COMPLIANCE:ACTION_CAPABILITIES.HANDLE_BUSINESS_COMPLIANCE,LEGAL:ACTION_CAPABILITIES.HANDLE_LEGAL}[requestedDomain]||null;
  }

  _activeCapabilities(identity){
    const authorityPermissions=identity?.authority?.action_permissions&&typeof identity.authority.action_permissions==="object"?identity.authority.action_permissions:{};
    const staffPermissions=identity?.permissions&&typeof identity.permissions==="object"?identity.permissions:{};
    const merged={...staffPermissions,...authorityPermissions};
