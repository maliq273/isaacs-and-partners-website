...
class PublicLeadLiaison {
    ...
    initialise() {
        ...
        this.restoreSession();
        this.bind();
        window.setTimeout(() => this.open("greeting"), 2200);
    }
            ...
            <div class="public-ai-liaison__panel" data-ai-panel hidden>
                <header class="public-ai-liaison__header">
                    <div class="public-ai-liaison__identity">
                        ...
                        <div><strong>Isaacs &amp; Partners</strong><span>AI Liaison</span></div>
                    </div>
    ...
    open(reason = "manual") {
        ...
        if (reason === "greeting" && !this.context && !this.list.children.length) {
    ...
    persistMessage(sender, body) {
        try {
            ...
            localStorage.setItem(STORAGE_KEY, JSON.stringify({ messages: messages.slice(-12), serviceName: this.serviceName }));
        } catch {
            // Optional local session only.
        }
    }
...
if (typeof document !== "undefined") {
    document.addEventListener("DOMContentLoaded", () => new PublicLeadLiaison().initialise());
}
