...
export default class HandoverService {
    assess({ intent = "UNKNOWN", message = "" } = {}) {
        const text = String(message).toLowerCase();
        const sensitive = SENSITIVE_TERMS.filter(term => text.includes(term));
        const humanRequired = HUMAN_INTENTS.has(intent) || sensitive.length > 0;
        return {
            humanRequired,
            reasons: [
                ...(HUMAN_INTENTS.has(intent) ? ["CLIENT_REQUEST_OR_OPERATIONAL_HANDOVER"] : []),
                ...(sensitive.length ? ["SENSITIVE_SUBJECT"] : [])
            ],
