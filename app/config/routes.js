...
export const ROUTE_NAMES = Object.freeze({ HOME:"HOME", LOGIN:"LOGIN", SIGNUP:"SIGNUP", DASHBOARD:"DASHBOARD", STAFF_DASHBOARD:"STAFF_DASHBOARD", SUPER_ADMIN_DASHBOARD:"SUPER_ADMIN_DASHBOARD", INDIVIDUAL_DASHBOARD:"INDIVIDUAL_DASHBOARD", BUSINESS_DASHBOARD:"BUSINESS_DASHBOARD", ACCOUNTS:"ACCOUNTS", CLIENTS:"CLIENTS", MATTERS:"MATTERS", REPORTS:"REPORTS", STAFF:"STAFF", STAFF_ADMIN:"STAFF_ADMIN", AI:"AI", ANALYTICS:"ANALYTICS", CONSULTATION:"CONSULTATION", BOOKING:"BOOKING", CLIENT_PORTAL:"CLIENT_PORTAL", NOT_FOUND:"NOT_FOUND" });
...
export const ROUTE_DEFINITIONS = Object.freeze([
    ...
    {name:ROUTE_NAMES.ACCOUNTS,path:ROUTES.ACCOUNTS,public:false,protected:true,meta:{title:"Accounts | Isaacs and Partners"}},
...
export function resolveRoute(route,base=""){if(!route)return base||"/";const b=String(base).replace(/\/+$/,""),r=String(route).replace(/^\/+/,"");return b?`${b}/${r}`:`/${r}`;}
export function getApiRoute(resource,id=null){const base=API_ROUTES[resource]||resource;return id?`${ROUTES.API}/${base}/${encodeURIComponent(id)}`:`${ROUTES.API}/${base}`;}
